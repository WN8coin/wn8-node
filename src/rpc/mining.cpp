...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "WN8 is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "WN8 is downloading blocks...");

    if (Params().GetConsensus().IsMain() && !masternodeSync.IsSynced())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "WN8 Core is syncing with network...");
