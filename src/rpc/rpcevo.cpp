...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
			...
			"\nCreates, funds and sends a ProTx to the network. The resulting transaction will move " + FormatMoney(ZNODE_REQUIRED_AMOUNT) + " WN8\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
	if (!fMasternodeAccess)
		throw JSONRPCError(RPC_VERIFY_ERROR, "Forbidden access");
	...
	CAmount collateralAmount = ZNODE_REQUIRED_AMOUNT;
