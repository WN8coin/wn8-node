...
UniValue encryptwallet(const JSONRPCRequest& request)
{
	...
	return "wallet encrypted; WN8 server stopping, restart to run with encrypted wallet.";
}
...
UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
			+ HelpExampleCli("settxfee", "0.00000001 " + CURRENCY_UNIT)
			+ HelpExampleRpc("settxfee", "0.00000001 " + CURRENCY_UNIT)
        );
