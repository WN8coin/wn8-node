            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if(nCoinType == CoinType::ALL_COINS){
                    ...
                    found = (pcoin->tx->vout[i].scriptPubKey.IsZerocoinMint()
                            ...
                            || pcoin->tx->vout[i].scriptPubKey.IsLelantusMint()
                            || pcoin->tx->vout[i].scriptPubKey.IsLelantusJMint());
				} else if (nCoinType == CoinType::ONLY_NOT5000IFMN) {
					found = !(fMasternodeMode && pcoin->tx->vout[i].nValue == ZNODE_REQUIRED_AMOUNT);
				} else if (nCoinType == CoinType::ONLY_NONDENOMINATED_NOT5000IFMN) {
					if (fMasternodeMode) found = pcoin->tx->vout[i].nValue != ZNODE_REQUIRED_AMOUNT; // do not use Hot MN funds
				} else if (nCoinType == CoinType::ONLY_5000) {
					found = pcoin->tx->vout[i].nValue == ZNODE_REQUIRED_AMOUNT;
                } else {
                    found = true;
                }
                if (!found) continue;
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
					(!IsLockedCoin((*it).first, i) || nCoinType == CoinType::ONLY_5000) &&
...
bool CWallet::GetZnodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
	...
	coinControl.nCoinType = CoinType::ONLY_5000;
