...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &params, int nTime) {

	if (nHeight == 0) return 0 * COIN;

	CAmount subsidy = 10.85 * COIN;

	if (nTime > ZC_GENESIS_BLOCK_TIME && nTime <= params.nFirstSubsidyHalvingEndPeriod) return subsidy;

	subsidy = subsidy - (subsidy * params.nFirstSubsidyHalvingValue) / 100;

	if (nTime > params.nFirstSubsidyHalvingEndPeriod && nTime <= params.nSecondSubsidyHalvingEndPeriod) return subsidy;

	subsidy = subsidy - (subsidy * params.nSecondSubsidyHalvingValue) / 100;

	if (nTime > params.nSecondSubsidyHalvingEndPeriod && nTime <= params.nThirdSubsidyHalvingEndPeriod) return subsidy;

	subsidy = subsidy - (subsidy * params.nThirdSubsidyHalvingValue) / 100;
...
CAmount GetMasternodePayment(int nHeight, int nTime, CAmount blockValue)
{
	...
	CAmount nodesReward = blockValue * params.nSubsidyHalvingNodesShare / 100;
	return nodesReward - nodesReward * params.nSubsidyHalvingDevelopShare / 100;
}
...
*/
	...
	if (chainActive.Tip()->GetBlockTime() == ZC_GENESIS_BLOCK_TIME)
		return false;
...
bool CheckZerocoinFoundersInputs(const CTransaction &tx, CValidationState &state, const Consensus::Params &params, int nHeight, int64_t nTime) {
    // Check for founders inputs
	if (nHeight > 0) {
		bool found = false;

		CAmount blockSubsidy = GetBlockSubsidy(nHeight, params, nTime);
		CScript FOUNDER_SCRIPT = GetScriptForDestination(CBitcoinAddress(params.subsidyDevelopFundAddress).Get());
		CAmount founderValue = blockSubsidy - blockSubsidy * params.nSubsidyHalvingMinersShare / 100;
		if (tx.vout.size() > 2) {
			founderValue = founderValue * params.nSubsidyHalvingDevelopShare / 100;
		}

		BOOST_FOREACH(const CTxOut &output, tx.vout) {
			if (output.scriptPubKey == FOUNDER_SCRIPT && output.nValue == founderValue) {
				found = true;
			}
		}
		...
		if (!found) {
            return state.DoS(100, false, REJECT_FOUNDER_REWARD_MISSING,
							 "CTransaction::CheckTransaction() : founder reward missing");
        }
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    */
	...
	LogPrintf("Write EVO Best Block \n");
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
	...
	if (pindexPrev && pindexPrev->nTime > ZC_GENESIS_BLOCK_TIME && block.nTime < ZC_GENESIS_BLOCK_TIME)
	...
	if (!CheckZerocoinFoundersInputs(*block.vtx[0], state, consensusParams, nHeight, block.nTime)) {
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
    CBlockIndex *pindex = NULL;
    {
        ...
        // belt-and-suspenders.
		bool ret = CheckBlock(*pblock, state, chainparams.GetConsensus(), true, true, pblock->nHeight);
