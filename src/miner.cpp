...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
	...
	bool fDIP0003Active_context = nHeight >= params.DIP0003Height;
	bool fDIP0008Active_context = nHeight >= params.DIP0008Height;
	...
	fIncludeWitness = IsWitnessEnabled(pindexPrev, params) && fMineWitnessTx;
	...
	CAmount nBlockSubsidy = GetBlockSubsidy(nHeight, params, pblock->nTime);
...
void BlockAssembler::FillFoundersReward(CMutableTransaction &coinbaseTx) {
	...
	if (nHeight > 0) {
		CAmount devPayoutValue = (GetBlockSubsidy(nHeight, params, pblock->nTime) * params.nSubsidyHalvingNodesShare) / 100;
		CScript devPayoutScript = GetScriptForDestination(CBitcoinAddress(params.subsidyDevelopFundAddress).Get());
    ...
    if (nHeight >= params.nSubsidyHalvingFirst && nHeight < params.nSubsidyHalvingFirst + params.nSubsidyHalvingInterval) {
        ...
        if (nHeight < params.nZnodePaymentsStartBlock) {
            ...
            if (params.IsMain() && (GetAdjustedTime() > nStartRewardTime)) {
				...
				FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("wLo2aLLW8ymWUmJuoA6eGXystrhQpR75Yb").Get());
				FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("wSUS6byMmGwngpr24nRFifLwfJk93BymdP").Get());
				FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("wa41a1LVGNtPyHBqYRcHESvJB3mbGmiogo").Get());
				FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("wM7UHGcXWyZ4HL751EJto6HTnBTtJV8Csy").Get());
				FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("wfDL5B9QgTBS2DmosQd4vTgWopRcn3seaV").Get());
            }
        ...
        while (true) {
            ...
            while (true) {
                ...
                while (true) {
					...
					} else if (!fTestNet && pindexPrev->nHeight >= 1) {
                        lyra2z_hash(BEGIN(pblock->nVersion), BEGIN(thash));
					} else if (fTestNet && pindexPrev->nHeight >= 1) { // testnet
                        lyra2z_hash(BEGIN(pblock->nVersion), BEGIN(thash));
					} else {
...
//                        LogPrintf("hashTarget: %s\n", hashTarget.ToString().c_str());
                    ...
                    if (powTarget <= hashTarget) {
                        ...
                        // Found a solution
						LogPrintf("Found a solution. Hash: %s\n", powTarget.ToString());
