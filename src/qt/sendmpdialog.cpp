...
void SendMPDialog::updateFrom()
{
    ...
    if (currentSetFromAddress.empty()) {
        ...
        if (CheckFee(currentSetFromAddress, 16)) {
			...
			ui->feeWarningLabel->setText("WARNING: The sending address is low on WN8 for transaction fees. Please topup the WN8 balance for the sending address to send Elysium transactions.");
