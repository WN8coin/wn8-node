...
void OverviewPage::handleEnabledTorChanged(){
    ...
    if(ui->checkboxEnabledTor->isChecked()){
		...
		msgBox.setText(tr("Please restart the WN8 wallet to route your connection through Tor to protect your IP address. <br>Syncing your wallet might be slower with TOR. <br>Note that -torsetup in wn8.conf will always override any changes made here."));
    }else{
        settings.setValue("fTorSetup", false);
		msgBox.setText(tr("Please restart the WN8 wallet to disable routing of your connection through Tor to protect your IP address. <br>Note that -torsetup in wn8.conf will always override any changes made here."));
    }
    msgBox.exec();
}
