...
{
	QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. WN8 can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
{
	migrateSettings("Zcoin", "Firo", "Zcoin-Qt", "WN8-Qt");
	migrateSettings("Zcoin", "Firo", "Zcoin-Qt-testnet", "WN8-Qt-testnet");
	...
	boost::filesystem::path firoDefaultDataDir = GetDefaultDataDirForCoinName("wn8");
    ...
    if (messageBox.exec() == QMessageBox::Yes) {
        if (RenameDirectoriesFromZcoinToFiro()) {
			...
			setDataDirInSettings("WN8", "WN8-Qt", GUIUtil::boostPathToQString(firoDefaultDataDir));
			setDataDirInSettings("WN8", "WN8-Qt-testnet", GUIUtil::boostPathToQString(firoDefaultDataDir));
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    // wn8: links repeatedly have their payment requests routed to this process:
