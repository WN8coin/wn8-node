...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
	...
	widget->setPlaceholderText(QObject::tr("Enter a WN8 address (e.g. %1)").arg(
...
{
	// return if URI is not valid or is no wn8: URI
	if(!uri.isValid() || uri.scheme() != QString("wn8"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
	// Convert wn8:// to wn8:
    //
	//    Cannot handle this later, because wn8:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
	if(uri.startsWith("wn8://", Qt::CaseInsensitive))
    {
		uri.replace(0, 10, "wn8:");
    }
...
{
	QString ret = QString("wn8:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
		return GetSpecialFolderPath(CSIDL_STARTUP) / "WN8.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
		return GetSpecialFolderPath(CSIDL_STARTUP) / "WN8 (testnet).lnk";
	return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("WN8 (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
		return GetAutostartDir() / "wn8.desktop";
	return GetAutostartDir() / strprintf("wn8-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
			...
			optionFile << strprintf("Name=WN8 (%s)\n", chain);
