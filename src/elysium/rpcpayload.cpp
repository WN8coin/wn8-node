...
UniValue elysium_createpayload_dexsell(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw runtime_error(
			...
			"\nCreate a payload to place, update or cancel a sell offer on the traditional distributed ELYSIUM/WN8 exchange.\n"
    ...
    if (action <= CMPTransaction::UPDATE) { // actions 3 permit zero values, skip check
		...
		amountDesired = ParseAmount(request.params[2], true); // WN8 is divisible
