...
unsigned char GetNfactor(int64_t nTimestamp) {
    int l = 0;
	if (nTimestamp <= ZC_GENESIS_BLOCK_TIME)
        return Params().GetConsensus().nMinNFactor;

	int64_t s = nTimestamp - ZC_GENESIS_BLOCK_TIME;
    while ((s >> 1) > 3) {
        l += 1;
        s >>= 1;
    }
...
bool CBlockHeader::IsProgPow() const {
	...
	return (nTime > ZC_GENESIS_BLOCK_TIME + 5/* && nTime >= Params().GetConsensus().nPPSwitchTime*/);
}
