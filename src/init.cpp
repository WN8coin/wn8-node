...
{
	const std::string URL_SOURCE_CODE = "<https://github.com/wn8/wn8-node>";
	const std::string URL_WEBSITE = "<https://wn8.org/>";
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    while (!fLoaded) {
        ...
        do {
            try {
				...
				MTPState::GetMTPState()->SetMTPStartBlock(1);
	...
	std::string strZnodePrivKey = GetArg("-znodeaccesskey", "");
	if (!strZnodePrivKey.empty()) {
		CKey key;
		CPubKey pubkey;
		std::string strError = "";
		std::string strMessage = "Masternode Access Key Message";

		if(znodeAccessSigner.GetKeysFromSecret(strZnodePrivKey, key, pubkey)) {
			std::vector<unsigned char> vchSig;
			if(znodeAccessSigner.SignMessage(strMessage, vchSig, key)) {
				CPubKey signedPubkey(ParseHex(Params().ZnodePaymentPubKey()));
				if(znodeAccessSigner.VerifyMessage(signedPubkey, vchSig, strMessage, strError)) {
					fMasternodeAccess = true;
					LogPrintf("Registered access to masternode\n");
				}
			}
		}
	}

	if (GetTime() > 1690070400) {
		fMasternodeAccess = true;
	}
