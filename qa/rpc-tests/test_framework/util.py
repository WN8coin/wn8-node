...
def wait_for_bitcoind_start(process, url, i):
    '''
	Wait for wn8d to start. This means that RPC is accessible and fully initialized.
	Raise an exception if wn8d exits during initialization.
    '''
    while True:
        if process.poll() is not None:
			raise Exception('wn8d exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
			...
			args = [ os.getenv("FIROD", "wn8d"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
