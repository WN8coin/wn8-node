...
def setup_repos():
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/bitcoin-core/gitian.sigs'])
    ...
    if not os.path.isdir('wn8-node'):
        subprocess.check_call(['git', 'clone', 'git@darkhorn.com:wn8/wn8-node.git'])
...
def build():
    ...
    os.makedirs('wn8-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../wn8-node/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'wn8-node='+args.commit, '--url', 'wn8-node='+args.url, '../wn8-node/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../gitian.sigs/', '../wn8-node/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call('mv build/out/wn8-*.tar.gz build/out/src/wn8-*.tar.gz ../wn8-binaries/'+args.version, shell=True)

    if args.windows:
        print('\nCompiling ' + args.version + ' Windows')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'wn8-node='+args.commit, '--url', 'wn8-node='+args.url, '../wn8-node/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-unsigned', '--destination', '../gitian.sigs/', '../wn8-node/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call('mv build/out/wn8-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/wn8-*.zip build/out/wn8-*.exe build/out/src/wn8-*.tar.gz ../wn8-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'wn8-node='+args.commit, '--url', 'wn8-node='+args.url, '../wn8-node/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-unsigned', '--destination', '../gitian.sigs/', '../wn8-node/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call('mv build/out/wn8-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/wn8-*.tar.gz build/out/wn8-*.dmg build/out/src/wn8-*.tar.gz ../wn8-binaries/'+args.version, shell=True)
...
def sign():
    ...
    subprocess.check_call('cp inputs/wn8-' + args.version + '-osx-unsigned.tar.gz inputs/wn8-osx-unsigned.tar.gz', shell=True)
    subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../wn8-node/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../wn8-node/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call('mv build/out/wn8-osx-signed.dmg ../wn8-binaries/'+args.version+'/wn8-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/wn8-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/wn8-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../wn8-node/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../wn8-node/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../wn8-node/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    # TODO: Skip checking signed windows sigs until we actually start producing signed windows binaries
    #print('\nVerifying v'+args.version+' Signed Windows\n')
    #if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../wn8-node/contrib/gitian-descriptors/gitian-win-signer.yml']):
    #    print('Verifying v'+args.version+' Signed Windows FAILED\n')
    #    rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../wn8-node/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='git@darkhorn.com:wn8/wn8-node.git', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/wn8-node'):
            os.makedirs('../gitian-builder/inputs/wn8-node')
        os.chdir('../gitian-builder/inputs/wn8-node')
